...
class Autosky(CustomAction):
    ...
    def run(
        ...
        while (
            ...
            for retry_count in range(MAX_RETRY_ATTEMPTS):
                ...
                if not context.run_recognition(
                    ...
                    logger.warning(
                        f"不在雷达界面，尝试重新进入 (重试 {retry_count+1}/{MAX_RETRY_ATTEMPTS})。"
                    )
                    ...
                    if not context.run_recognition(
                        ...
                        logger.warning(
                            f"未能回到雷达界面，跳过本次自动探索尝试 (重试 {retry_count+1}/{MAX_RETRY_ATTEMPTS})。"
                        )
                        time.sleep(2)  # 失败后等待一下
                        continue  # 继续下一次重试

                logger.info(
                    f"尝试离开雷达界面并开始自动探索 (重试 {retry_count+1}/{MAX_RETRY_ATTEMPTS})。"
                )
                time.sleep(1)  # 给一些时间让界面稳定
        ...
        if self._encountered_unbeatable:
            ...
            context.run_task(
                "ReturnBigMap"
            )  # 这里实际上是有问题的，因为先得从打不过的状态退出到雷达界面，再从雷达界面退出到伊甸大厅，再从伊甸大厅退出到伊甸，最后从伊甸退出到大地图。
            return CustomAction.RunResult(success=False)
        elif self._completed_exploration_rounds >= self._target_exploration_rounds:
            logger.info(
                f"Autosky 任务已成功完成全部 {self._target_exploration_rounds} 轮探索。"
            )
            send_message(
                "MaaGB",
